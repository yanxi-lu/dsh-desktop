...
// 单实例:未抢到锁的实例立即退出;其余启动代码全部包在 else 内,
// 避免输掉的实例继续注册 IPC/托盘并 spawn 出抢不到端口的 dsh(R22)。
...
if (!gotLock) {
  ...
  async function startAndShowMain(): Promise<{ ok: boolean; error?: string }> {
    ...
    try {
      ...
      // 中转窗可能已被 exit watch 关闭(spawn 后进程立即死),只防 null 会朝已销毁窗口 send(R22)
    ...
    tray = createTray(
      {
        onOpen: () => {
          if (mainWindow && !mainWindow.isDestroyed()) {
            ...
            && dshProc.proc.pid
            && !dshProc.proc.killed
            && dshProc.proc.exitCode === null
          ) {
            // 全部窗口已关闭且服务仍在:重建主窗
            openMainWindow();
          } else {
            // 服务已死(或从未启动):打开崩溃页,提供重启出口(R22)
            openCrash();
          }
        },
        onQuit: quit,
